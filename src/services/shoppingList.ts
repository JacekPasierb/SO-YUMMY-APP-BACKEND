...
const addIngredientToShoppingList = async (
  ...
  ingredientData: { ingredientId: string; thb: number; name: string; measure: string; recipeId: string }
) => {
...
const deleteIngredientFromShoppingList = async (
  ...
  await ShoppingList.updateOne(
    { userId },
    { $pull: { items: ingredientData } }
  );
};

export {getShoppingListByUserId, addIngredientToShoppingList, deleteIngredientFromShoppingList}